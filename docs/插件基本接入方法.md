...
var exampleActionBlock = new BlockMetadata
{
    ...
    Args = new Dictionary<string, MetaArgsBase>
    {
        ...
        ["ExampleDynamicDropdown"] = new DynamicDropdownMetaArgs
        {
            Name = "示例动态下拉框字段",
            Type = MetaType.dynamic_dropdown,
            Id = "sai.actions.example.options"
        }
    },
...
> 用于在前段加载时获取下拉框内容，而非 initialize 时的固定内容

位于 `ISaiServe.RegisterDynamicDropdown`，下方为定义：
...
```csharp
saiServerService.RegisterDynamicDropdown("sai.actions.example.options", () => [("???", "???")]);
```
