...
const data = await wsWaitMessage(ws, { type: 'getExtraBlocks' });
 ...
 */
export const runCode = async (workspace = window.workspace) => {
 ...
 * @param workspace 工作区实例
 */
export const saveCode = async (workspace = window.workspace) => {
    console.log(workspace);
    let code = javascriptGenerator.workspaceToCode(workspace);
    code = `(async () => {\n${code}\n})();\n`;
    code = await prettier.format(code, {
 ...
 */
export const injectBlockly = async (dom) => {
    ...
    if (workspace) {
        let data;
        try {
            data = await wsWaitMessage(ws, { type: "load", guid: projectUuid });
